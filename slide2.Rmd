...
fp <- getPop(sex = "females", years = 1968:2014)
frat <- fd / fp
md <- getDeaths(sex = "males")
mp <- getPop(sex = "males", years = 1968:2014)
mrat <- md / mp
cut <- 1:100
favg <- numeric(47)
mavg <- numeric(47)
...
fp <- getPop(sex = "females", years = 1968:2014, ages = 90:100)
frat <- fd / fp
md <- getDeaths(sex = "males", ages = 90:100)
mp <- getPop(sex = "males", years = 1968:2014, ages = 90:100)
mrat <- md / mp
cut <- 1:10
favg <- numeric(47)
mavg <- numeric(47)
...
par(lwd = 2)
plot(fit.c, axes = FALSE, xlab = "Age", ylab = "Cum. hazards")
tic <- 0:5
axis(1, at = tic, labels = tic + 110)
axis(2, las = 1)
box()
x <- seq(0, 5, length = 100)
shp <- fit.g$coefficients["rate"]
scl <- exp(fit.g$coefficients["log(level)"])
y <- Hgompertz(x, shape = 1/shp, scale = scl)
y <- scl * exp(x / shp)
lines(x, y)
...
##dr = drop1(fit, test = "Chisq")
##ltx(fit, dr = dr)
plot(fit, fn = "sur", col = c("red", "blue"))
```
