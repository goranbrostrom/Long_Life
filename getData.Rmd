...
```{r styronper}
load("~/Forskning/read_U18006/data/dors.rda")
indx <- match(per$LINNEID, dors$LINNEID)
##per$DODSDAT <- dors$DODSDAT[indx]
per$DODSDAT <- per$birthdate + dors$ALDER[indx] + 0.5 ## Because DODSDAT is missing here!
...
fob <- fob[, c("LINNEID", "AR", "KOMMUN")]  ## Note!!
...
save(fobdata, file = "fobdata.rda")
```

# The Lisa data

The *lisa* data is slightly simpler because it is yearly with cutoff dates 12-31/01-01 each new-year. It starts off at 1987-01-01 (lisa1986) and ends at 2013-12-31 (lisa2013). So the time interval a *lisa* covers is *AR + 1*! But note that the *dors* data only cover deaths *before January 1, 2014, so *lisa2013* is in effect useless to us.  

```{r lisadat}
rm(fobdata)
load("~/Forskning/read_U18006/data/lisa.rda")
lisa <- lisa[, c("LINNEID", "AR", "KOMMUN")]
...
save(lisa, file = "lisadata.rda") ## NOTE!!
```
